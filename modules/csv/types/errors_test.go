...
func TestCSVError(t *testing.T) {
	t.Run("row only", func(t *testing.T) {
		err := newCSVError(5, "something went wrong")
	...
	t.Run("row and column", func(t *testing.T) {
		err := &CSVError{Row: 3, Column: 7, Msg: "unterminated quote"}
	...
	t.Run("formatted error", func(t *testing.T) {
		err := newCSVErrorf(12, "expected %d fields but got %d", 5, 7)
	...
	t.Run("unwrap", func(t *testing.T) {
		...
		err := &CSVError{Row: 1, Msg: "wrapped", Err: inner}
