...
func assertRuntimeDateTimeLocation(t *testing.T, value runtime.Value, expected string) runtime.DateTime {
	t.Helper()

	dt, ok := value.(runtime.DateTime)
	if !ok {
		t.Fatalf("expected runtime.DateTime, got %T", value)
	}

	if dt.Location().String() != expected {
		t.Fatalf("expected location %q, got %q", expected, dt.Location().String())
	}
