...
func (fn *Function) withArg(arg cdpruntime.CallArgument) *Function {
	if fn.args == nil {
		fn.args = make([]cdpruntime.CallArgument, 0, defaultArgsCount)
	}
...
func (fn *Function) eval(ctx cdpruntime.ExecutionContextID) *cdpruntime.CallFunctionOnArgs {
	exp := fn.prepExp()

	call := cdpruntime.NewCallFunctionOnArgs(exp).
...
func (fn *Function) compile(ctx cdpruntime.ExecutionContextID) *cdpruntime.CompileScriptArgs {
	exp := fn.precompileExp()

	call := cdpruntime.NewCompileScriptArgs(exp, "", true)
