...
func (s *NavigationEventStream) Read(ctx context.Context) <-chan runtime.Message {
...
func (s *NavigationEventStream) Close() error {
	val := s.tail.Load()
	...
	ec, err := eval.Create(s.ctx, s.logger, s.client, evt.FrameID)
