...
`
	...
	Convey(".GetNodeType", t, func() {
		...
		el, err := memory.NewHTMLElement(doc, doc.Find("body"))
	...
	Convey(".Value", t, func() {
		...
		el, err := memory.NewHTMLElement(doc, doc.Find("#q"))
	...
	Convey(".GetInnerText", t, func() {
		...
		el, err := memory.NewHTMLElement(doc, doc.Find("h2"))
	...
	Convey(".InnerHtml", t, func() {
		...
		el, err := memory.NewHTMLElement(doc, doc.Find("#content"))
	...
	Convey(".QuerySelector", t, func() {
		...
		el, err := memory.NewHTMLElement(doc, doc.Selection)
	...
	Convey(".XPath", t, func() {
		Convey("Text nodes", func() {
			...
			el, err := memory.NewHTMLElement(doc, doc.Find("html"))
		...
		Convey("Attributes", func() {
			...
			doc, err := memory.NewRootHTMLDocument(godoc, "localhost:9090")
			So(err, ShouldBeNil)
